...
const INTENT_MAP: Record<string, { category: string; subcategory?: string }> = {
...
const SUBCATEGORY_ENTITY_MAP: Record<string, string> = {
...
function pickTopIntent(intents: WitIntent[] | undefined): WitIntent | null {
  if (!intents || intents.length === 0) return null
  let best = intents[0]
  for (const i of intents) if (i.confidence > best.confidence) best = i
  return best ?? null
}

function sortIntents(intents: WitIntent[] | undefined): WitIntent[] {
  if (!intents) return []
  return [...intents].sort((a, b) => (b.confidence ?? 0) - (a.confidence ?? 0))
}

function pickEntitySuggestion(entities: WitMessageResponse['entities']): Suggestion | null {
  if (!entities) return null

  let bestCategory: { value: string; confidence: number } | null = null
  let bestSub: { value: string; confidence: number } | null = null

  for (const key of Object.keys(entities)) {
    // Wit typically returns arrays of objects.
    const arr = entities[key]
    if (!Array.isArray(arr)) continue
    for (const item of arr as WitEntityValue[]) {
      ...
      if (mappedCategory) {
        if (!bestCategory || c > bestCategory.confidence) bestCategory = { value: mappedCategory, confidence: c }
      }

      const mappedSub = SUBCATEGORY_ENTITY_MAP[normalized]
      if (mappedSub) {
        if (!bestSub || c > bestSub.confidence) bestSub = { value: mappedSub, confidence: c }
      }

      // Back-compat: allow entities to directly return our slugs.
      if (CATEGORY_SLUGS.has(v)) {
        if (!bestCategory || c > bestCategory.confidence) bestCategory = { value: v, confidence: c }
      }
      if (SUBCATEGORY_SLUGS.has(v)) {
        if (!bestSub || c > bestSub.confidence) bestSub = { value: v, confidence: c }
      }
    }
  }

  if (!bestCategory || bestCategory.confidence < 0.6) return null
...
function pickSubcategoryFromEntities(entities: WitMessageResponse['entities']): { subcategory: string; confidence: number } | null {
  if (!entities) return null
  ...
  for (const key of Object.keys(entities)) {
    ...
    for (const item of arr as WitEntityValue[]) {
      ...
      if (!best || c > best.confidence) best = { value: mapped, confidence: c }
    }
  }
...
function toSuggestion(top: WitIntent): Suggestion | null {
  ...
  if (mappedCategory) return { category: mappedCategory, confidence: top.confidence }
...
export function registerIntakeRoutes(router: Router) {
  router.get('/intake/suggest', async (req, res) => {
    ...
    if (!config.witServerAccessToken) {
      const payload: IntakeSuggestResponse = { enabled: false, suggestion: null, alternatives: [] }
    ...
    try {
      ...
      if (!r.ok) {
        const payload: IntakeSuggestResponse = { enabled: true, suggestion: null, alternatives: [] }
        return res.json(payload)
      }

      body = (await r.json()) as WitMessageResponse
    } catch {
      const payload: IntakeSuggestResponse = { enabled: true, suggestion: null, alternatives: [] }
      return res.json(payload)
    }
    ...
    if (entityCategorySuggestion) {
      const payload: IntakeSuggestResponse = { enabled: true, suggestion: entityCategorySuggestion, alternatives: [] }
      return res.json(payload)
    }
    ...
    const merged: Suggestion | null = topCategory
      ? {
