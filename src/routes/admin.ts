...
import {
  ...
  adminAnalyticsRecentQuerySchema,
...
export function registerAdminRoutes(router: Router) {
  ...
  router.get('/admin/analytics/overview', async (req, res) => {
    ...
    try {
      const totalR = await pool.query<{ total: number }>(
      ...
      const perDayR = await pool.query<{ day: string; count: number }>(
      ...
      const topPathsR = await pool.query<{ path: string; count: number }>(
  ...
  router.get('/admin/analytics/recent', async (req, res) => {
    ...
    try {
      const r = await pool.query<{ path: string; referrer: string; createdAt: string }>(
        `select path,
                referrer,
  ...
  router.get('/admin/contacts', async (_req, res) => {
    const r = await pool.query(
      `select id, subject, pii_encrypted as "piiEncrypted", created_at as "createdAt"
  ...
  router.post('/admin/upload', upload.single('image'), async (req, res) => {
    const f = req.file
    if (!f) return res.status(400).json({ message: 'Imagen requerida (jpg/png/webp/gif, máx 3MB)' })
  ...
  router.get('/admin/news/sources', async (_req, res) => {
    const r = await pool.query(
      'select id, name, rss_url as "rssUrl", enabled, created_at as "createdAt" from news_sources order by created_at desc'
    )
    return res.json({ items: r.rows })
  })

  router.post('/admin/news/sources/clear', async (req, res) => {
    const confirm = Boolean((req.body as any)?.confirm)
    if (!confirm) return res.status(400).json({ message: 'Confirmación requerida' })
  ...
  router.post('/admin/news/sources', async (req, res) => {
    ...
    try {
      const r = await pool.query(
        'insert into news_sources(name, rss_url, enabled) values ($1,$2,true) returning id',
        [name, rssUrl]
      )
  ...
  router.get('/admin/news/posts', async (_req, res) => {
    const r = await pool.query(
      'select id, title, content, image_url as "imageUrl", featured, template, coalesce(media,\'[]\'::jsonb) as "media", link, source, published_at as "publishedAt", created_at as "createdAt" from news_posts order by published_at desc limit 200'
    )
    return res.json({ items: r.rows })
  })
  ...
  router.post('/admin/news/posts', async (req, res) => {
    ...
    const r = await pool.query(
      `insert into news_posts(title, content, image_url, featured, template, media, link, source, published_at)
       values ($1,$2,$3,$4,$5,$6::jsonb,$7,$8,$9)
       returning id`,
      [title, content ?? '', imageUrl || null, isFeatured, t, JSON.stringify(m), link || '', 'admin', pub]
    )
  ...
  router.delete('/admin/news/posts/:id', async (req, res) => {
    ...
    const imageUrl = prev.rows[0]?.imageUrl as string | null | undefined
    if (imageUrl) {
  ...
  router.put('/admin/home/hero', async (req, res) => {
    ...
    if (slides.length > 7) return res.status(400).json({ message: 'Máximo 7 fotos' })
    ...
    try {
      ...
      for (let i = 0; i < slides.length; i++) {
        const s = slides[i]
        await pool.query(
          `insert into home_hero_slides(position, image_url, title, text, link, enabled, updated_at)
           values ($1,$2,$3,$4,$5,$6, now())`,
          [i + 1, s.imageUrl, s.title ?? '', s.text ?? '', s.link ?? '', s.enabled ?? true]
        )
      }
    ...
    router.put('/admin/news/carousel', async (req, res) => {
      ...
      const slides = parsed.data.slides
      if (slides.length > 12) return res.status(400).json({ message: 'Máximo 12 fotos' })
      ...
      try {
        ...
        for (let i = 0; i < slides.length; i++) {
          const s = slides[i]
          await pool.query(
            `insert into news_carousel_slides(position, image_url, title, text, link, enabled, updated_at)
             values ($1,$2,$3,$4,$5,$6, now())`,
            [i + 1, s.imageUrl, s.title ?? '', s.text ?? '', s.link ?? '', s.enabled ?? true]
          )
        }
