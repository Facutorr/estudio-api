...
import { adminAnalyticsOverviewQuerySchema, adminAnalyticsRecentQuerySchema, adminReviewsQuerySchema, homeHeroSlidesUpsertSchema, newsCarouselSlidesUpsertSchema, newsPostCreateSchema, newsSourceCreateSchema, productCreateSchema, productUpdateSchema, productsListQuerySchema } from '../validation.js';
...
export function registerAdminRoutes(router) {
    ...
    router.post('/admin/products', async (req, res) => {
        ...
        try {
            const r = await pool.query(`insert into products(name, description, category, price, stock, images, sizes, colors, featured, active)
         values ($1, $2, $3, $4, $5, $6::jsonb, $7::jsonb, $8::jsonb, $9, $10)
         returning id`, [name, description, category, price, stock, JSON.stringify(images), JSON.stringify(sizes), JSON.stringify(colors), featured, active]);
            return res.json({ ok: true, id: r.rows[0].id });
        }
        catch (err) {
    ...
    router.put('/admin/products/:id', async (req, res) => {
        ...
        values.push(id);
        try {
            const result = await pool.query(`update products set ${fields.join(', ')} where id = $${paramIndex} returning id`, values);
    ...
    router.delete('/admin/products/:id', async (req, res) => {
        ...
        try {
            ...
            const ordersCheck = await pool.query('select id from order_items where product_id = $1 limit 1', [id]);
            ...
            // Clean up images
            const images = prev.rows[0]?.images;
