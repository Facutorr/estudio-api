...
export function registerNewsRoutes(router) {
    ...
    router.get('/news', async (_req, res) => {
        ...
        const posts = await pool.query('select id, title, content, image_url as "imageUrl", featured, template, coalesce(media,\'[]\'::jsonb) as "media", link, source, published_at as "publishedAt" from news_posts order by published_at desc limit 50');
        ...
        const sources = await pool.query('select name, rss_url as "rssUrl" from news_sources where enabled = true order by created_at desc');
