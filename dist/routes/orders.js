...
export function registerOrdersRoutes(router) {
    ...
    router.post('/orders', requireAuth, async (req, res) => {
        ...
        const { shippingName, shippingEmail, shippingPhone, shippingAddress, shippingCity, shippingDepartment, shippingPostalCode, notes } = parsed.data;
        try {
            // Get cart items
            const cartResult = await pool.query(`select ci.id,
         ...
         where ci.user_id = $1`, [userId]);
          ...
          returning id`, [
            ...
            values ($1, $2, $3, $4, $5, $6, $7)`, [orderId, item.productId, item.productName, item.productPrice, item.quantity, item.size, item.color]);
                    // Decrease stock
                    await pool.query('update products set stock = stock - $1, updated_at = now() where id = $2', [item.quantity, item.productId]);
                }
                // Clear cart
    ...
    router.get('/orders/:id', requireAuth, async (req, res) => {
        ...
        const userId = req.user.sub;
        try {
            const orderResult = await pool.query(`select id,
                status,
                total,
                shipping_name as "shippingName",
                shipping_email as "shippingEmail",
         ...
         where id = $1 and user_id = $2`, [id, userId]);
            ...
            // Get order items
            const itemsResult = await pool.query(`select id,
